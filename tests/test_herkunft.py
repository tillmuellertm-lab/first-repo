...
def test_offene_punkte_werden_nach_ursache_gebuendelt(tmp_path, capsys):
    """Gleiche Fehlanzeige, unterschiedlich geschrieben, gehoert in eine Zeile."""
    import argparse
    ...
    for nummer, fehlend in enumerate(fehlanzeigen):
        dokument, _ = mappe.datei_aufnehmen(
            _datei(tmp_path, f"beleg{nummer}.txt", inhalt=str(nummer)), herkunft_jahr=2024
        )
        dokument.analyse = Analyse(
            kategorie_id="werbungskosten_sonstige",
            eignung="bedingt",
            betrag_gesamt=10.0,
            fehlende_nachweise=[fehlend],
        )
    # Ein Beleg aus einem anderen Jahr darf die Buendelung nicht verfaelschen.
    fremd, _ = mappe.datei_aufnehmen(_datei(tmp_path, "fremd.txt", inhalt="x"), herkunft_jahr=2025)
    fremd.analyse = Analyse(
        kategorie_id="werbungskosten_sonstige", fehlende_nachweise=["Zahlungsnachweis fehlt"]
    )
    mappe.speichern()

    befehl_offen(argparse.Namespace(mappe=str(mappe.wurzel), jahr=None))
    ausgabe = capsys.readouterr().out

    assert "4 Dokumente aus 2024" in ausgabe
    zeilen = [z for z in ausgabe.splitlines() if "Zahlungsnachweis fehlt" in z]
    assert len(zeilen) == 1, "die drei Schreibweisen gehoeren in eine Zeile"
    assert zeilen[0].strip().startswith("3"), "und zwar mit Anzahl 3, nicht 4"
